...
module {
  // OldActor = NewActor of 20260524_300000_AddDrawings
  ...
  // Old ControllerProfile (without passwordHash)
  ...
  // New ControllerProfile (with passwordHash)
  type NewControllerProfile = {
    var name         : Text;
    var email        : Text;
    var phone        : Text;
    var passwordHash : Text;
  ...
  type NewActor = {
    ...
    users              : Map.Map<Text, UserRecord>;
    resetCodes         : Map.Map<Text, ResetCodeEntry>;
    controllerProfile  : NewControllerProfile;
    loginSessions      : Map.Map<Principal, List.List<SessionEntry>>;
    featureLocks       : FeatureLockState;
    bankAccountStore   : { var bankAccount : ?BankAccount };
    payoutStore        : { var payoutRecords : List.List<PayoutRecord>; var nextPayoutDate : ?Text };
    ...
    reviewRequestsStore    : Map.Map<Text, ReviewRequest>;
    drawingNotifications   : Map.Map<Text, DrawingNotification>;
    // New field: additional login emails for the controller
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // Migrate ControllerProfile: carry over name/email/phone, init passwordHash to empty
      controllerProfile  = {
      ...
      // Initialize additional login emails as empty list
      loginEmails        = List.empty<Text>();
    };
  };
};
