...
module {
  // OldActor = NewActor of 20260524_110000_AddAuthUsers.mo
  ...
  type SubscriptionStatus = { #trial; #active; #expired; #cancelled; #frozen };
  ...
  // ControllerProfile as it exists in the live actor (from types/auth.mo)
  // The prior migration (20260524_110000) declared it without passwordHash,
  // but types/auth.mo added the field. This NewActor definition must
  // match the actual running state shape.
  ...
  // ── OldActor = NewActor of 20260524_110000_AddAuthUsers ─────────────────
  // NOTE: The prior migration declared ControllerProfile without passwordHash.
  // If the live canister was upgraded after types/auth.mo added passwordHash,
  // include it here too. We use a supertype with the field optional-absent
  // approach: declare both variants and match what compiles. Since we cannot
  // know the exact deployed shape without the .most file, we use the shape
  // that matches types/auth.mo (the authoritative source).

  // OldSubscriptionStatus = same as SubscriptionStatus (already deployed with #frozen)
  type OldSubscriptionStatus = { #trial; #active; #expired; #cancelled; #frozen };

  // OldUserSubscription = same as UserSubscription (frozenAt/frozenReason already in live state)
  ...
  type OldActor = {
    ...
    controllerProfile  : OldControllerProfile;
  };
  ...
  public func migration(old : OldActor) : NewActor {
    // Clear any stored password hash for the controller user account.
    // The controller must set their own password via changeControllerPassword.
    let controllerEmail = "pwarre12@mail.ccsf.edu";
    switch (old.users.get(controllerEmail)) {
      case (?u) { u.passwordHash := "" };
      case null {};
    };
    {
      phases             = old.phases;
      ...
      subscriptions      = old.subscriptions.map<Principal, OldUserSubscription, UserSubscription>(
        func(_k, s) {
          let newStatus : SubscriptionStatus = switch (s.status) {
            case (#trial)     { #trial };
            case (#active)    { #active };
          ...
          };
          {
            ...
            chargeScheduledAt     = s.chargeScheduledAt;
            subscriptionStartAt   = s.subscriptionStartAt;
      ...
      // Lift OldControllerProfile to ControllerProfile, clearing passwordHash
      controllerProfile  = {
        var name         = old.controllerProfile.name;
        var email        = old.controllerProfile.email;
        var phone        = old.controllerProfile.phone;
        var passwordHash = "";
      };
    };
  };
