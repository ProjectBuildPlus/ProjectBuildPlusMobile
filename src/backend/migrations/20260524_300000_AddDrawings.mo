...
module {
  // OldActor = NewActor of 20260524_200000_AddPayout
  ...
  // ReviewRequest includes oacFormId from the start (consolidated from 400000)
  type ReviewRequest = {
    id          : Text;
    drawingId   : Text;
    requestedBy : Text;
  ...
  type NewActor = {
    ...
    bankAccountStore   : { var bankAccount : ?BankAccount };
    payoutStore        : { var payoutRecords : List.List<PayoutRecord>; var nextPayoutDate : ?Text };
    // New drawings fields (ReviewRequest includes oacFormId)
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // Initialize new drawings fields
      drawingsStore        = Map.empty<Text, Drawing>();
      reviewRequestsStore  = Map.empty<Text, ReviewRequest>();
      drawingNotifications = Map.empty<Text, DrawingNotification>();
    };
  };
};
